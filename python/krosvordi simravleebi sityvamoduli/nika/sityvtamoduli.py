def tsinadadebis_damushaveba(tsinadadeba):
    ...
    """
    ts_list = tsinadadeba.split(" ")
    return ts_list
#test
# print(f'წინადადების გახლეჩის შედეგი: {tsinadadebis_damushaveba("1 2 2131 123 1312")}')
...
def sityvis_damushaveba(striqoni):
    ...
    """
    sityva_down = striqoni.lower()
    return sityva_down
#test
#print(f"სიტყვის ყველა ასოს დაბალზე გადაყვანის შედეგი: {sityvis_damushaveba('BACKEND')}")
...
def sityvis_analizi(sityva):
    ...
    """
    leqsikoni = dict()
    print(f"სიტყვა: {sityva}")
    for i in range(len(sityva)):
        aso = sityva[i]
        print(f"sityva[i]: {sityva[i]}")
        # if leqsikoni.get(sityva[i]) == None:
        if leqsikoni.get(aso) == None:
            # leqsikoni[sityva[i]] = i
            leqsikoni[aso] = [i]

        else:
            # leqsikoni[aso] = f"{leqsikoni[sityva[i]]}, {i}"

            leqsikoni[aso].append(i)
    return leqsikoni
#გამაწვალა რა, სანამ გავაკეთე, 7 ჯერ თავიდან დავწერე ;დ
